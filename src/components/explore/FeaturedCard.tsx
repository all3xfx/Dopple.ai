...
export function FeaturedCard({ data }: FeaturedCardProps) {
  return (
    <>
      <Link
        className={cn(
          "relative flex max-h-[260px] min-h-[260px] flex-col overflow-hidden rounded-[15px] bg-black5 shadow-lg2",
      ...
      >
        <div className="h-[260px] w-full overflow-hidden">
          {data.bannerURL && (
            <Image
              className={cn(
                "object-cover transition duration-[0.8s] group-hover:scale-[1.05]",
              )}
              fill
              src={data.bannerURL}
              sizes="(min-width: 768px) 25vw, (min-width: 414px) 50vw, 100vw"
              alt=""
            />
          )}
        </div>
        <div className={cn("absolute h-full w-full bg-gradient6")}>
          <div
            className={cn(
              "absolute left-[15px] top-[15px] flex items-center space-x-[5px] rounded-[5px] bg-inputback p-[5px] text-[12px] font-medium leading-[15px] md:text-[14px] md:leading-[17px]",
            )}
          >
            {categories[data.category].image && (
              <Image
                className={cn("h-[15px] w-[15px] md:h-auto md:w-auto")}
          ...
          <div
            className={cn(
              "absolute bottom-[15px] left-[15px] flex w-[calc(100%-30px)] items-center space-x-[10px]",
            )}
          >
            <div
              className={cn(
                "flex w-0 flex-1 items-center space-x-[5px] text-[16px] font-bold leading-[19px]",
              )}
            >
              {data.avatarURL && (
                <Image
              ...
              <Tooltip
                content={
                  <Fragment>
                    <div
                      className={cn("flex flex-col items-start space-y-[15px]")}
                    >
                      <span
                        className={cn(
                      ...
                      <div className={cn("flex items-start space-x-[10px]")}>
                        <Image
                          width={15}
                          height={15}
                          className={cn("h-[15px] w-[15px] md:h-5 md:w-5")}
            ...
            <div
              className={cn(
                "flex items-center space-x-[5px] text-[16px] font-semibold leading-[19px]",
              )}
            >
              <svg
